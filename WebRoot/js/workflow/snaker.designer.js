(function(snaker){
	...
	designer.rect=function(p,m){
		...
		snaker([t.node,f.node,n.node,e.node]).bind("click",function(){
			...
			switch(o){
				case"pointer":break;
				case"sequence":var r=snaker(C).data("currNode");
	...
	designer.props=function(h,f){
		var j=this,c=snaker("#props").hide().draggable({handle:"#title"}).resizable().css(designer.config.props.attr).bind("click",function(){return false}),e=c.find("table"),g=f,i;
		var d=function(n,m,o){
			...
			for(var l in m){
				...
				e.append("<tr><td class='prop_name'>"+m[l].label+"</td><td class='prop_value'><div id='p"+l+"' class='editor'></div></td></tr>");
				if(m[l].editor){
	...
	designer.editors={
		textEditor:function(){
			...
			this.init=function(i,h,m,l,j){
				d=i;e=h;c=m;g=l;f=j;
				snaker('<input type="text">').val(g.text()).change(function(){
	...
	designer.init=function(x,r){
		...
		if(designer.config.editable){
			snaker("#tools").draggable({handle:"#title"}).css(designer.config.tools.attr);
			snaker("#tools .node").hover(function(){snaker(this).addClass("mover")},function(){snaker(this).removeClass("mover")});
			snaker("#tools .select_able").click(function(){
				snaker(".selected").removeClass("selected");
				snaker(this).addClass("selected");
				snaker(y).data("mod",this.id)
			});
			snaker("#tools .state").each(function(){
